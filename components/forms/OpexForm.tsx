      ...
      <div className="space-y-2">
        ...
        {ox.staff.map((line, idx)=> (
          <div key={idx} className="grid md:grid-cols-[1fr_1fr_auto] items-center gap-3">
            <input className="input" value={line.role}
              onChange={(e)=> {
                const staff = [...ox.staff];
                staff[idx] = { ...staff[idx], role: e.currentTarget.value };
                set({ staff });
              }} />
            <input className="input" value={line.monthly.toLocaleString("id-ID")}
              onChange={(e)=> {
                const staff = [...ox.staff];
            ...
            <button className="text-sm underline text-red-300" title="Delete" onClick={()=> {
              const staff = ox.staff.filter((_,i)=> i!==idx);
              set({ staff });
            }}>Remove</button>
          </div>
        ))}
        <button className="button-share btn-sm" onClick={()=> set({ staff: [...ox.staff, { role: "New Role", monthly: 5_000_000 }] })}>Add Staff</button>
      </div>
